...
export default function CategoryClaims() {
  ...
  const countCommentsRecursively = (
    commentList: Comment[]
  ): { total: number; forCount: number; againstCount: number } => {
    let totalCount = 0;
    let forCount = 0;
    let againstCount = 0;
  ...
  const getCommentsWithReplies = async (
    ...
    try {
      ...
      if (allCommentsError) {
        console.error(
          'Error fetching comments for claim',
          claimId,
          ':',
          allCommentsError
        );
      ...
      const topLevelComments =
        commentsData?.filter((comment) => comment.parent_comment_id === null) ||
        [];
  ...
  const fetchCategoryClaims = async () => {
    try {
      ...
      const claimCreatorIds =
        categoryClaims?.map((claim: any) => claim.op_id).filter(Boolean) || [];
          ...
          return {
            ...
            forPercentage:
              commentStats.total > 0
                ? Math.round((commentStats.forCount / commentStats.total) * 100)
                : 0,
            againstPercentage:
              commentStats.total > 0
                ? Math.round(
                    (commentStats.againstCount / commentStats.total) * 100
                  )
                : 0,
